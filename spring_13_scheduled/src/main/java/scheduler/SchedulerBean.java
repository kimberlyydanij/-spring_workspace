 ...
 * 
 * 로그(log) 프로그램 개발이나 운영시 문제점을 추적하거나 운영상태를 모니터링
 * 하기 위한 텍스트이다.
 ...
 * 
 * INFO로 세팅하면  INFO,WARN,ERROR,FATAL을 기록한다.
 * 
 * FATAL : 심각한에러 -> 프로그램 자체가 실행되지 않을 수 있다.
 * ERROR : 문제가 발생하면 소프트웨어 일부가 기능을 수향하지 못할 수 있다.
 * WARNING : 소프트웨어가 동작하고 있지만, 예상치 못한 일이 발생하고 가까운 미래에 문제가
 * 				   발생할 수 있다는 경고이다.  EX) 디스크 공간 부족
 * INFO : 작업이 정상적으로 작동하고 있음을 메세지를 통해 확인하고 싶은 경우에 사용
...
public class SchedulerBean {
	...
	public void SchedulerRun() {
		...
		//System.out.println("스케쥴링 실행22: " + dateFormat.format(calendar.getTime()));
	} // end scheduler run
